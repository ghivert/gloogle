...
fn view_search_input(model: Model) {
  ...
  h.form(
    [a.class("search-wrapper"), e.on_submit(fn(_) { msg.UserSubmittedSearch })],
    [
      ...
      search_input.view(state.loading, state.input, small: False),
      h.input([
        a.class("search-submit"),
        a.type_("submit"),
        a.value("Submit"),
        a.disabled(state.loading || !has_content),
      ]),
    ],
  )
}
...
fn sidebar(model: Model) {
  let state = bright.state(model)
  use <- bool.guard(when: state.is_mobile, return: el.none())
  let opacity = case state.route {
    router.Search(..) -> a.style("opacity", "1")
    _ -> a.style("opacity", "0.3")
  }
  let pointer_events = case state.route {
    router.Search(..) -> a.none()
    _ -> a.style("pointer-events", "none")
  }
  h.main([a.class("search-sidebar")], [
    ...
    h.form([e.on_submit(fn(_) { msg.UserSubmittedSearch })], [
      search_input.view(state.loading, state.input, small: True),
    ]),
    h.div([a.class("sidebar-filter"), opacity, pointer_events], [
      el.text("Filters"),
    ]),
    h.div([a.class("sidebar-filters"), opacity, pointer_events], [
      checkbox(state.keep_functions, msg.Functions, "Functions"),
      checkbox(state.keep_types, msg.Types, "Types"),
      checkbox(state.keep_aliases, msg.Aliases, "Aliases"),
      h.div([a.class("filter-separator")], []),
      checkbox(state.keep_documented, msg.Documented, "Documented"),
      checkbox(
        state.show_old_packages,
        msg.ShowOldPackages,
        "Show old versions",
      ),
      h.div([a.class("filter-separator"), opacity, pointer_events], []),
      checkbox(
        state.show_documentation_search,
        msg.DocumentationSearch,
        "Documentation Search",
      ),
      checkbox(state.show_vector_search, msg.VectorSearch, "Vector Search"),
    ]),
    h.div([a.class("sidebar-spacer"), opacity, pointer_events], []),
...
fn popularity_chart(model: Model) {
  let state = bright.state(model)
  let data =
    list.filter(state.popular, fn(p) {
...
fn analytics_chart(model: Model) {
  let state = bright.state(model)
  let data = state.timeseries
  use <- bool.guard(when: list.is_empty(data), return: el.none())
  chart.line_chart({
    let acc = Dataset([], [])
    use Dataset(dates, value), #(count, date) <- list.fold(data, acc)
    let #(date, _) = timestamp.to_calendar(date, calendar.utc_offset)
      ...
      True ->
        h.div(
          [
            ...
            a.style("background", "var(--border-color)"),
          ],
          [],
        )
    },
  ...
  el.fragment([
    ...
    h.main([a.class("main"), a.style("padding", "24px 36px")], [
      ...
      h.div([a.class("analytics-box-wrapper")], [
        analytics_box("Number of searches", state.total_searches),
        analytics_box("Number of signatures indexed", state.total_signatures),
        analytics_box("Number of packages indexed", state.total_packages),
      ]),
...
fn view_packages(model: Model) {
  ...
  el.fragment([
    ...
    h.main(
      ...
      list.intersperse(
        {
          use package <- list.map(state.packages)
...
pub fn body(model: Model) {
  ...
  case state.route {
    ...
    router.Search(_) -> {
      let key = model.search_key(state.submitted_input, state)
      el.fragment([
        sidebar(model),
        case
          dict.get(state.search_results, key)
          |> result.unwrap(search_result.Start)
        {
          ...
          search_result.SearchResults(_, _, _, _, _, _) -> {
            dict.get(state.view_cache, key)
            |> result.map(lazy.lazy)

  ...
  |> promise.tap(fn(res) {
    case res {
      ...
      Ok(res) -> {
        ...
        case option.map(discuss.decoder, decode.run(res.body, _)) {
          option.Some(Error(error)) -> discuss.on_error(DecodeError(error))
          option.Some(Ok(res)) -> discuss.on_success(res)
          option.None -> Nil
        }
      }
    }
